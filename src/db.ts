...
export class TiDBClient {
  ...
  async search(
    ...
    if (isAutoMode) {
      distanceExpression = "VEC_EMBED_COSINE_DISTANCE(embedding, ?) AS distance";
      params.push(query);
    } else {
      const [queryVector] = await this.embeddingProvider.embed([query]);
      const vectorToUse = queryVector && queryVector.length > 0
        ? queryVector
        : new Array(this.embeddingProvider.dimension).fill(0);
      distanceExpression = "VEC_COSINE_DISTANCE(embedding, VEC_FROM_TEXT(?)) AS distance";
      params.push(JSON.stringify(vectorToUse));
    }
  ...
  private async insertBatchWithRetry(batch: ChunkItem[]): Promise<number> {
